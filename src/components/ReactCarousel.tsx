      ...
      >
        {
          items.map(({ name }, i) => {
            return (
              <li
                className={`cursor-pointer state whitespace-nowrap ${isAdminInterface ? 'sm:h-[35px]' : ''} ${isCompetitiveAnalysis ? 'sm:h-[35px]' : ''} ${isHome ? 'h-[35px] sm:w-[125px] sm:h-[40px]' : 'sm:h-[27px] h-[35px] '} ${isFinalAccount ? 'sm:w-[125px] sm:h-[40px]' : ""} ${isProductsFilters ? 'h-[35px] sm:h-[40px] sm:w-[135px]' : ''} ${isCheckout ? "h-[35px] w-[100px] sm:w-[135px] sm:h-[40px]" : ""} ${isPrototypes ? 'sm:h-[35px] px-6' : ''} xl:h-[41px] grid place-items-center rounded-full`}

...
export default function ReactCarousel({ items, children, handleClick, index, isQuizzical, cardBgColor, isCompetitiveAnalysis, isAdminInterface, isHome, isFinalAccount, isProductsFilters, isCheckout }: Props) {
  ...
  const productsFiltersStyles = isProductsFilters ? "!text-[10px] sm:!text-[14px]" : ""
  const checkoutStyles = isCheckout ? "!text-[10px] sm:!text-[14px] sm:w-[304px] justify-between" : ""
      ...
      >
        {
          items.map(({ name }, i) => {
            return (
              <li
                className={`cursor-pointer state whitespace-nowrap ${isHome ? 'w-[71px] h-[35px] sm:w-[125px] sm:h-[40px]' : 'sm:h-[27px] h-[46px] '} ${isFinalAccount ? 'w-[90px] sm:w-[125px] sm:h-[40px]' : ""} ${isProductsFilters ? 'sm:h-[40px] sm:w-[135px]' : ''} ${isCheckout ? "w-[100px] sm:w-[135px] sm:h-[40px]" : ""} xl:h-[41px] grid place-items-center rounded-full px-[16px]`}

...
export default function ReactCarousel({ items, children, handleClick, index, isQuizzical, cardBgColor, isCompetitiveAnalysis, isAdminInterface, isHome }: Props) {
  ...
  return (
    <>
      <ul
        className={`mx-auto font-['Jost'] flex-row ${isHomeStyles} sm:text-[14px] xl:text-[16px] font-medium overflow-scroll items-center border-[2px] sm:border-[3px] rounded-full p-[2px] h-max-content uppercase scrollbar-0 tracking-wide ${competitiveAnalysisStyles} ${quizzicalStyles} ${thriftstudioStyles}`}
          ...
          items.map(({ name }, i) => {
            return (
              <li
                ...
                className={`cursor-pointer state whitespace-nowrap ${isHome ? 'w-[71px] h-[35px] sm:w-[125px] sm:h-[40px]' : 'sm:h-[27px] h-[46px] '} xl:h-[41px] grid place-items-center rounded-full px-[16px]`}
